...
class SaleImportRule(Component):
    ...
    def _get_paid_amount(self, record):
        payment_adapter = self.component(
            usage="backend.adapter", model_name="__not_exist_prestashop.payment"
        )
...
class SaleOrderImportMapper(Component):
    ...
    def total_tax_amount(self, record):
        tax = float(record["total_paid_tax_incl"]) - float(
            record["total_paid_tax_excl"]
        )
    ...
    def _import_dependencies(self):
        ...
        for row in rows:
            try:
                ...
                _logger.error(
                    "PrestaShop product %s could not be imported, "
                    "error: %s",
                    row["product_id"],
                    err,
                )
            ...
            if row.get("product_attribute_id", "0") != "0":
                try:
                    self._import_dependency(
                        row["product_attribute_id"], "prestashop.product.combination"
                    )
                except PrestaShopWebServiceError as err:
